 ...
 // initialize the peripherals & the board GPIO
	  ...
	  while(1)
    {
      RTC_GetDateTime(&rtc);//get current date & time stamp
			...
			sprintf((char *)msg,"\ntime:%2d:%2d:%2d  Date:%2d/%2d/%2d \x0d\xa",
				(uint16_t)rtc.hour,(uint16_t)rtc.min,(uint16_t)rtc.sec,(uint16_t)rtc.date,
		  ...
		  //Weight Measurement
			...
			else if(pinent==12)   //end of the bill
			{
				total+=cost;
				if(total>=500)
					total-=100;
